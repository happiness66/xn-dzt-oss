...
function buildList(options) {
    ...
    tableEl.bootstrapTable({
        ...
        queryParams: function(params) {
            ...
            options.searchParams.start = params.offset / params.limit + 1;
            options.searchParams.limit = params.limit;
            for (var p in searchFormParams) {
                if (!searchFormParams[p]) {
                    delete searchFormParams[p];
                }
            }
            sessionStorage.setItem('listSearchs', JSON.stringify(searchFormParams));
            console.log(sessionStorage.getItem('listSearchs'));
